...
import d3 from 'd3'
import DonutChartLegend from '../donut-chart-legend'
...
export default class DonutChart extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            total_contribution: this.props.data[0]?!isNaN(this.props.data[0].total_contribution)?this.props.data[0].total_contribution: this.props.data[0].budget ? this.props.data[0].budget : this.props.data[0].theme_budget:0
        }
    }
    componentWillReceiveProps(nextProps) {
        this.setState({
            ...
            total_contribution: nextProps.data[0]?!isNaN(nextProps.data[0].total_contribution)?nextProps.data[0].total_contribution: nextProps.data[0].budget ? nextProps.data[0].budget : nextProps.data[0].theme_budget:0
        },
            () => {
    ...
    drawDonut = () => {
        ...
        const onHover = (d) => {
            this.setState({
                ...
                total_contribution: !isNaN(d.data.total_contribution)?d.data.total_contribution: d.data.budget ? d.data.budget : d.data.theme_budget
            }, () => {
        ...
        var svg = d3.select('#' + this.props.chart_id)
            ...
            .classed(`${style.svgIe} ${this.props.svgIe}`, true)
            .append("g")
            .attr("transform",
        ...
        return (
            <div class={`${style.donut_wrapper} ${this.props.donor_wrapper_styles}`}>
                <div class={`${style.donut} ${this.props.donut_styles}`} id={this.props.chart_id}>
                    ...
                    {
                        !this.props.exportLegend && (percentage != 0 || percentage != "") ? 
                            <div class={this.props.percDiv}>
                               <section class={`${style.textLabel} ${this.props.textWrapperStyle}`} >
                                {sdg_code ? <span class={style.imageWrapper}><img class={style.sdg_image} src={`../../../assets/icons/${sdg_src}`} alt="sdg image"/></span> : null}
                                <span class={`${style.perc} ${this.props.donutCenterText}`}>
                                    {percentage == parseInt(percentage)?
                                        parseFloat(percentage).toFixed(1)+ '%' 
                                        : (percentage >= 1) ? parseFloat(percentage).toFixed(1) + '%' : '< 1%'
                                    }
                                </span>
                                {
                                    total_contribution ?
                                    ...
                                    :
                                    <span class={`${style.perc} ${style.contributionText} ${this.props.donutCenterText}`}>{numberToCurrencyFormatter(percentage)}</span>
                                }
                             </section> 
                ...
                {
                    this.props.displayLegend && this.props.displayLegend == 'false' ? null : <div class={`${style.legend} ${this.props.legend_styles} ${this.props.ourFocusLegendStyles}`}>
                        <DonutChartLegend legendLabel={this.props.legendLabel} legendData={this.props.legendData} />
                    </div>
