...
from undp_outputs.models import Output, OutputParticipatingOrganisation, OutputLocation, OutputSector, Budget, \
    ...
    Expense, ActivityDate, OutputSdg, OutputActiveYear
...
class ProcessOutput:
    ...
    def process(self, obj):
        ...
        try:
            with transaction.atomic():
                ...
                self.process_sdg_nodes(output, obj.findall('sector'))
            ...
            save_log(message='Output Error: %s' % e, type=LOG_TYPE.output, key=output_id)
    ...
    def output_active_years(self, output, year):
        try:
    ...
    def do(self):
        ...
        for row in range(sheet.nrows):
            if row_iter != 0:
                ...
                try:
                    operating_unit = OperatingUnit.objects.get(iso2=iso2)
                except:
                    operating_unit = None

                try:
                    document_category = DocumentCategory.objects.get(category=category)
                except Exception as e:
                    print(e)
...
class DownloadXmlCron:
    ...
    download_url = settings.ZIP_DOWNLOAD_URL
    ...
    def do(self):
        ...
        try:
            ...
            download_zip_file_from_location(self.download_url, self.download_filepath, self.annual_xml_filepath)
            end_time = datetime.datetime.now()
            run_time = end_time - start_time
            print('Runtime : ' + str(run_time))
            print('Cron Ended')
...
class DifferenceAnalysisCron:
    ...
    def processXML(self, file_names):
        ...
        for xml_file in file_names:
            ...
            projects = []
            outputs = []
            unique_yearly_projects = []
            unique_yearly_outputs = []
            for event, obj in iter_obj:
                i = i + 1
                # IATI hierarchy used to determine if output or project
                ...
                if hierarchy == '1':
                    ...
                    if project_id not in unique_projects:
                        ...
                        unique_yearly_projects.append(project_id)
                elif hierarchy == '2':
                    o += 1
                    output_key = processXmlIterObject(obj, {'iati-identifier'}, None, 2)
                    output_id = getIdFromKey(output_key['text'])
                    outputs.append(output_id)
                    if output_id not in unique_outputs:
                        tuo += 1
                        yuo += 1
                        unique_outputs.append(output_id)
                        unique_yearly_outputs.append(output_id)

            print("")
            print("============================")
            print("Year: %s" % str(current_year))
            print("Total yearly records: %s" % str(i))
            print("Total projects: %s" % str(p))
            print("Total outputs: %s" % str(o))
            print("Yearly unique projects: %s" % str(yup))
            print("Yearly unique outputs: %s" % str(yuo))
            print("============================")
            message += "\n============================\n"
            m1 = "Year: %s\n" % (str(current_year), )
            m2 = "Total yearly records: %s" % str(i)
            m3 = "Total projects: %s" % str(p)
            m4 = "Total outputs: %s" % str(o)
            m5 = "Yearly unique projects: %s" % str(yup)
            m6 = "Yearly unique outputs: %s" % str(yuo)
            m7 = "Yearly unique project id: %s" % unique_yearly_projects
            m8 = "Yearly unique output id: %s" % unique_yearly_outputs
            message += m1 + "\n" + m2 + "\n" + m3 + "\n" + m4 + "\n" + m5 + "\n" + \
                m6 + "\n" + m7 + "\n" + m8
            message += "\n============================\n"
        print("============================")
        print("Total unique projects: %s" % str(tup))
        print("Total unique outputs: %s" % str(tuo))
        print("============================")
        m9 = "Total unique projects: %s" % str(tup)
        m10 = "Total unique outputs: %s" % str(tuo)
        message += "\n============================\n" + \
                   m9 + "\n" + m10 + "\n============================\n"
    ...
    def write_file(self, content):
        file_path = settings.LOG_UPLOAD_DIR + "/analysis.txt"
        f = open(file_path, "a+")
        f.write(content)
        f.write("\r\n")
        f.close()
...
class UpdateSearchModel:
    ...
    def do(self):
        project_entries = list(ProjectActiveYear.objects.all())
        project_search_items = []
        for entry in project_entries:
            mappings = DonorFundSplitUp.objects.filter(project=entry.project, year=entry.year)
            sectors = OutputSector.objects.filter(project=entry.project)
            sdgs = OutputSdg.objects.filter(project=entry.project)
        ...
        ProjectSearch.objects.all().delete()
